    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (userID.trim().equals(curUser.getUserID().trim())) {
                request.setAttribute("DELETE_ERROR", "You can't delete yourself!!!");
                url = ERROR + "?search=" + lastSearch;
            } else {
                UserDAO dao = new UserDAO();
                boolean check = dao.delete(new UserDTO(userID, "", "", ""));
                if (check) {
                    url = SUCCESS + "?search=" + lastSearch;
                }
            }

        } catch (Exception e) {
            log("Error at DeleteController: " + e.toString());
            request.setAttribute("DELETE_ERROR", "An error has occured!!");
//            url = ERROR + "?search=" + lastSearch;
        } finally {
